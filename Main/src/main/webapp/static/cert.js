        ...
        success: function (result) {
            ...
            if (result.data_success) {
                ...
                $.each(data, function (i, n) {
                    ...
                    content += '	  <button onclick="deleteuser(' + n.id + ',\'' + n.name + '\')" id="btn-danger" value="' + n.name + '" type="button" class="btn btn-danger btn-xs"><i class=" glyphicon glyphicon-remove"></i></button>';
                    content += '  </td>';
                    content += '</tr>';
                });
...
function deleteUserBach() {
    ...
    if (SelectCheckbox.length == 0) {
        layer.confirm("删除数据不能为空?", {icon: 3, title: '提示'})
        return;
    }
